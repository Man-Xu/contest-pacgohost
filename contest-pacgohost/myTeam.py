...
class DummyAgent(CaptureAgent):
    ...
    def getdangerfoods(self, gameState):
        foodlist = self.getFood(gameState).asList()
        dangerfoods = [
            food for food in foodlist if food in self.dangerfoodlist]
        return dangerfoods
    ...
    def getclosestgoast(self, gameState):
        ...
        ene = [e for e in enemies if not e.isPacman and e.getPosition()
               != None]
        dist = 9999
        ghost = None
        for e in ene:
    ...
    def getclosestenemy(self, gameState, defence=False):
        ...
        enemies = [gameState.getAgentState(i)
                   for i in self.getOpponents(gameState)]
        if defence:
            ene = [e for e in enemies if e.isPacman and e.getPosition()
                   != None]
        else:
            ene = [e for e in enemies if not e.isPacman and e.getPosition()
                   != None]
        if len(ene) > 0:
            dists = [self.getMazeDistance(
                selfpos, a.getPosition()) for a in ene]
            return min(dists)
        else:
            return 9999
...
class possearchproblem:
    def __init__(self, gameState, agent):
        ...
        self.ene = [e.getPosition() for e in enemies if not e.isPacman and e.getPosition(
        ) != None and e.scaredTimer == 0]
...
class invaderproblem(possearchproblem):
    def __init__(self, gameState, agent):
        ...
        enemies = [gameState.getAgentState(agentIndex)
                   for agentIndex in agent.getOpponents(gameState)]
        self.invaders = [enemy.getPosition(
        ) for enemy in enemies if enemy.isPacman and enemy.getPosition() != None]
...
class keepdisproblem(possearchproblem):
    def __init__(self, gameState, agent):
        ...
        enem = [gameState.getAgentState(i)
                for i in agent.getOpponents(gameState)]
        self.inv = [enemy.getPosition()
                    for enemy in enem if enemy.isPacman and enemy.getPosition() != None]
        self.agent = agent
